...
class OPTAttention_mant(nn.Module):
    ...
    def quantize_value(self, value_states: torch.Tensor, org_v_shape: torch.Tensor, group_size: int, bsz: int, q_len: int) -> torch.Tensor:
        ...
        v_seq_len = value_states.shape[2] # (b, n, seq, d)
        ...
        if org_v_shape[-2] == 1:
            ...
            if v_seq_len % group_size == 0:
                ...
                value_states = value_states.reshape(v_cache_shape[0], v_cache_shape[2], v_cache_shape[1], v_cache_shape[3]).transpose(1, 2).contiguous()
            else:
    ...
    def forward(
        ...
        # query_states (B, S, N*D)
        # key_states, value_states shape (B, N, S, D)
        if self.quant_attention == True:
            org_v_shape = value_states.shape # (B, N, S, D)
            query_states, key_states = self.quantize_query_key(query_states, key_states, self.group_size)
            # Reshape to 4D (B, N, S, D) for quantize_value
            value_states = value_states.reshape(bsz, self.num_heads, tgt_len, self.head_dim)
            value_states = self.quantize_value(value_states, org_v_shape, self.group_size, bsz, tgt_len)
        ...
        if self.quant_attention == True:
            ...
            # attn_probs = pseudo_quantize_int(attn_probs, n_bit=self.q_bit, q_group_size=self.group_size)
            # attn_probs = attn_probs.reshape(org_probs_shape)

            attn_probs = self.quantize_attention_weights(attn_probs, self.group_size)
        ...
        if self.quant_attention == True:
            # org_outputs_shape = attn_output.shape
            # attn_output = attn_output.reshape(attn_output.shape[1], -1)
            # attn_output = pseudo_quantize_int(attn_output, n_bit=self.q_bit, q_group_size=self.group_size)
            # attn_output = attn_output.reshape(org_outputs_shape)
...
class OPTDecoderLayer_mant(nn.Module):
    def __init__(self, config: OPTConfig):
        super().__init__()
        self.embed_dim = config.hidden_size

        self.self_attn = OPTAttention_mant(config=config, is_decoder=True)
...
class OPTDecoder_mant(OPTPreTrainedModel):
    """
    ...
    def __init__(self, config: OPTConfig):
        ...
        self.layers = nn.ModuleList([OPTDecoderLayer_mant(config) for _ in range(config.num_hidden_layers)])
...
class OPTModel_mant(OPTPreTrainedModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
        self.decoder = OPTDecoder_mant(config)
        # Initialize weights and apply final processing
        self.post_init()
...
class OPTForCausalLM_mant(OPTPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = OPTModel_mant(config)
