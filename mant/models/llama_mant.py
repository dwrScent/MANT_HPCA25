...
from ..quantize.quant_func import pseudo_quantize_mant
...
class LlamaAttention_mant(nn.Module):
    ...
    def _init_rope(self):
        if self.config.rope_scaling is None:
            self.rotary_emb = LlamaRotaryEmbedding_mant(
                self.head_dim,
    ...
    def quantize_query_key(self, query_states: torch.Tensor, key_states: torch.Tensor, group_size: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        query_states = query_states.reshape(query_states.shape[1], -1) # (b, n, s, d) -> (n, b*s*d)
        key_states = key_states.reshape(key_states.shape[1], -1) # (b, n, s, d) -> (n, b*s*d)

        # Quantize query and key states
        query_states = pseudo_quantize_int(query_states, n_bit=self.q_bit, q_group_size=group_size)
        ...
        key_states = pseudo_quantize_mant(key_states, q_group_size=group_size)
    ...
    def quantize_value(self, value_states: torch.Tensor, org_v_shape: torch.Tensor, group_size: int, bsz: int, q_len: int) -> torch.Tensor:
        ...
        if org_v_shape[-2] == 1:
            ...
            v_cache_shape = value_states.shape  # value_states.shape (b, n, s, d)
            ...
            if v_seq_len % group_size == 0:
                ...
                quantized_part_deq = pseudo_quantize_mant(value_trans[:, (v_seq_len-group_size):], q_group_size=group_size)

                value_trans = torch.cat([value_trans[:, :(v_seq_len-group_size)], quantized_part_deq], dim=1)

                # reshape to org_v_shape
            ...
            # quantize the V vector to INT8
                ...
                # self.v_channel_scale is calculated in prefill stage
        ...
        # Prefill stage
            ...
            # For the following INT8 quantization in decode stage
            self.v_channel_scale = value_trans.abs().amax(dim=1, keepdim=True) / 127

            quantized_part_deq = pseudo_quantize_mant(quantized_part, q_group_size=group_size)
    ...
    def quantize_attention_weights(self, attn_weights, group_size):
        ...
        attn_weights = pseudo_quantize_int(attn_weights, n_bit=self.q_bit, q_group_size=group_size)
    ...
    def quantize_attention_output(self, attn_output, group_size):
        ...
        attn_output = pseudo_quantize_int(attn_output, n_bit=self.q_bit, q_group_size=group_size)
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Quantize Q and K after rotary embedding
...
class LlamaRMSNorm_mant(nn.Module):
...
ALL_LAYERNORM_LAYERS.append(LlamaRMSNorm_mant)

class LlamaMLP_mant(nn.Module):
    ...
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            down_proj = self.down_proj(self.act_fn(self.gate_proj(x)).to(self.up_proj.weight.device) * self.up_proj(x))


        return down_proj
    
class LlamaDecoderLayer_mant(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size

        self.self_attn = LlamaAttention_mant(config=config, layer_idx=layer_idx)

        self.mlp = LlamaMLP_mant(config)
        self.input_layernorm = LlamaRMSNorm_mant(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = LlamaRMSNorm_mant(config.hidden_size, eps=config.rms_norm_eps)
...
class LlamaModel_mant(LlamaPreTrainedModel):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer_mant(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        self.norm = LlamaRMSNorm_mant(config.hidden_size, eps=config.rms_norm_eps)
...
class LlamaForCausalLM_mant(LlamaPreTrainedModel):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = LlamaModel_mant(config)
        self.vocab_size = config.vocab_size
